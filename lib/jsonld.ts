...
export function buildArticleJsonLd(a: FeatureArticle): Record<string, unknown> {
  const url = `${BASE}/feature/${encodeURIComponent(a.id)}`;
  ...
  // Only emit dateModified if a distinct updated date is available
  if (
    "updated" in a &&
    typeof a.updated === "string" &&
    a.updated &&
    a.updated !== a.date
  ) {
    jsonLd.dateModified = a.updated;
  }
  return jsonLd;
}
...
export function buildFeatureBreadcrumbJsonLd(
  ...
  return buildBreadcrumbJsonLd([
    ...
    { name: a.title, url: `${BASE}/feature/${encodeURIComponent(a.id)}` },
  ]);
}
...
export function buildFeatureItemListJsonLd(
  ...
  return {
    "@context": "https://schema.org",
    "@type": "ItemList",
    "@id": `${BASE}/feature/${encodedId}#itemlist`,
    name: a.title,
    itemListElement: a.ranking.map((item, index) => ({
      ...
      image: item.images?.[0]?.startsWith("http")
        ...
        : item.images?.[0]
          ? `${BASE}${item.images[0]}`
          : undefined,
      url: item.href ? `${BASE}${item.href}` : `${BASE}/feature/${encodedId}#spot-${index + 1}`,
    })),
  };
}
