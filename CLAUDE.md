...
> ## 🚨 スプレッドシート / 記事URL / 自動生成パイプラインを触る前に必ず読む
> - [`automation/記事URL運用ルール.md`](automation/記事URL運用ルール.md)
>   … 列構成・行ズレの原理・URL表記の絶対ルール・禁止スクリプト・復旧手順
> - [`automation/申し送り.md`](automation/申し送り.md)
>   … 未処理タスク・パイプライン全ステップ・「ログを信じるな」の実例・直した不具合一覧
>
> **完了判断は `node scripts/check-gaps.mjs`（成果物の実在）で行う。
> 「未処理0件」「成功N件」の表示は実際に3回嘘をついた。**
