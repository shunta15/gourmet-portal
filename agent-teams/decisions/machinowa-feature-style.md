...
> 🚨 **記事本文の構成テンプレ（タイトル / 導入文 / POINT×5 / 編集部のひとこと / 最後に、歩き方）は [machinowa-article-template.md](./machinowa-article-template.md) を必ず参照すること。**
> このファイルは「トーン・画像・ID・割愛ルール」を扱い、本文の書き方テンプレはあちらに集約している。
...
## 🚨 ID／URL 命名規約（2026-05-24 ユーザー明言・確定）
...
- URL で問題になる記号は除外: `。` `/` `(` `)` `?` `&` `#` `%`
- 旧 `/feature/teleapo-feat-*` は middleware.ts で 301 リダイレクト救済済（既存6記事のみ）
