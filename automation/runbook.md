...
1. **GBP の住所が確認できない時は記事を作らない**（場所推測禁止）
2. **店舗名から場所・業種・客層を推測することは絶対禁止**
3. **画像は実店舗のもののみ**（Unsplash 等の汎用画像 NG）
4. **失礼な表現禁止**: spec.md §14 の禁止表現リスト遵守
5. **noindex のまま**: FEATURES 一覧に載せない、URL だけ存在
6. **1 回の実行で最大 3 件まで**（暴走防止）
7. **各処理の間は最低 5 秒空ける**
